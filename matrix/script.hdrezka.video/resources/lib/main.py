    ...
    def build(self):
        ...
        if len(self.info) == 0:
            ...
            for s, e in self.info.items():
                ...
                season = season.replace('{{ label }}',f'{lg_seas} {s}')
                

                items = []
                for i in e:
                    episode = episFile
                    episode = episode.replace('{{ label }}',f'{lg_epis} {i}')
                    episode = episode.replace('{{ onclick }}',f'RunScript(script.hdrezka.video,call=episode,set={s}_{i})')
        ...
        with open(self.skinFile, 'w', encoding="utf-8") as sf:
            sf.write(skin)
